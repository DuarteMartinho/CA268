...
class BST:
    ...
    def recursive_present(self, ptr, itemRequested):
        if ptr == None:  # Base Case
            return False
        else:
            item = ptr.item
            ...
            if item == itemRequested:
                return item == itemRequested
            else:
                return self.recursive_present(ptr.left, itemRequested) or self.recursive_present(ptr.right, itemRequested)

    def is_present(self, item):
        return self.recursive_present(self.root, item)
    
    def recursive_leafcount(self, ptr):
        if ptr == None:  # Base Case
            return 0
        else:
            if ptr.left == None and ptr.right == None:
                return 1 + self.recursive_leafcount(ptr.left) + self.recursive_leafcount(ptr.right)
            else:
                return self.recursive_leafcount(ptr.left) + self.recursive_leafcount(ptr.right)

    def count_leaves(self):
        return self.recursive_leafcount(self.root)